...
module ExercisesDay1 =
    ...
    let StringFormattingRevisited() =
        ...
        let getFunFacts x =
            let doubled = x * 2
            let tripled = x * 3
            $"{x} doubled is {doubled}, and {x} tripled is {tripled}!"
    ...
    let ComputerTheAverageNumberOfLetters() =
        ...
        let result = initialValue |> List.map (fun x -> x.Length) |> List.map (fun x -> float x) |> List.average
    ...
    let TestIfEverWordContainsTheLetterA() =
        let test (l: list<string>) =
            l |> List.forall (fun (x: string) -> x.Contains("a"))
    ...
    let TestIfAWordHasNoRepeatingLetter() =
        let test (s: string) =
            List.ofSeq s |> List.distinct |> List.length = s.Length
    ...
    let SumSquareDifference() =
        ...
        let sumOfSquares = [ 1 .. 10 ] |> List.sum |> (fun x -> x * x)
        let squareOfTheSum = [ 1 .. 10 ] |> List.map (fun x -> x * x) |> List.sum
    ...
    let FindTheSumOfAllTheMultiples() =
        ...
        let result = List.init 1000 (fun x -> x) |> List.filter (fun x -> x % 3 = 0 || x % 5 = 0) |> List.sum
